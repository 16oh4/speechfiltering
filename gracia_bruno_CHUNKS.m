...
duration = 50;          %how long should signal be (seconds)
begin = 15;              %reference point to start (seconds)
chunkSize = 5;     %how many seconds per chunk to do convolution
filePath = "noise_11.wav"; %file path to .wav relative to home directory
...
chunkSamples = sampleRate*chunkSize; %constant
chunksTotal = ceil(duration/chunkSize);
totChunkSamples = chunksTotal*chunkSamples;
chunks = (begin*sampleRate) : chunkSamples : (begin*sampleRate)+totChunkSamples;
remSamples = totChunkSamples-length(lchannel);
...
disp("");
%{
%mark the bounds of each chunk
chunks = (begin*sampleRate): (chunkSamples): (begin+duration)*sampleRate; 
chunkSampleSize = chunks(length(chunks)) - chunks(1);

%check for uneven chunks
remChunks = mod(chunkSampleSize, chunkSamples);

%If wav sample does not divide evenly into chunks, then add remaining
if remChunks > 0
    chunks = [chunks [chunks(length(chunks))+chunkSamples] ];
end

%chunkSampleSize = chunks(length(chunks)) - chunks(1);

%add zeros to complete chunks
if length(lchannel) < chunkSampleSize
    zeroPad = chunkSampleSize - length(lchannel);
    lchannel = [lchannel zeros(1, zeroPad)];
    rchannel = [rchannel zeros(1, zeroPad)];
else
    zeroPad = 0;
    lchannel = lchannel(1: chunkSampleSize);
    rchannel = rchannel( 1 : chunkSampleSize);
end
%}
...
%timeAxis =  begin:(1/sampleRate):(begin+duration + ((length(filter)-2)/sampleRate));
